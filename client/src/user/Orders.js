...
const UserOrders = () => {
  ...
  const preload = () => {
    axios
      .get(`${API}/order/user/orders/${user._id}`, {
        headers: {
                ...
                <div className="row text-center mb-2 py-1 px-2 bg-dark ">
                  <div className="row">
                      ...
                      <ul className="list-unstyled">
                        {order.orders &&
                          order.orders.map((product, index) => {
                            return (
                              <li className="text-white mt-1" key={index}>
                                {product.name}{" "}
                                <span className="ml-1 text-danger">Price:</span>
                                {product.price}
                              </li>
                            );
                          })}
                    ...
                    <div className="col-md-4 col-xs-12">
                      <p
                        aria-disabled={true}
                        className={`text-white btn  bg-${getbackgroundColor(
                          order.status
                        )}`}
                      >
                        {order.status}
  ...
  return <Base title="Your orders">{OrdersPage()}</Base>;
};
